...
(function() {
    ...
    LC.RectangleWidget = function(_super) {
        ...
        RectangleWidget.prototype.options = function() {
            var $el, $input, $super, _this = this;
            $el = $("      <input type='checkbox' class='transparent-val'>    ");
            $input = $el.filter("input");
            if ($input.size() === 0) {
                $input = $el.find("input");
            }
            this.tool.input = $input;
            $super = RectangleWidget.__super__.options.apply(this, arguments).hide();
            $input.bind("change", function(e) {
                _this.tool.transparent = $input.attr("checked");
